<template>
  <div class="top">
    <div class="column"><i class="el-icon-video-camera-solid" />记录 ({{ state.videos.length }})</div>
    <div class="column">
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      ...
      videos: computed(() => video.myVideo.history),
      edit: false
    })

    const load = page => {
      dispatch('video/myVideo/fetchHistory', page)
    }

    const deleteVideo = (id, index) => {
      dispatch('video/myVideo/deleHistory', { id, index })
    }
    ...
    onMounted(() => {
      dispatch('video/myVideo/fetchHistory', 1)
    })
...
<style scoped lang="scss" src="/@/sass/video/myVideo/historyVideos.scss"></style>
