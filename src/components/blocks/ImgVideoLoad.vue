<template>
  <div v-if="playIcon" class="mask">
    <div class="play" />
  </div>
  <div :style="{ minHeight: minHeight }" :class="['image', { error2: state.imgError }]">
    ...
    const state = reactive({
      ...
      imgApiUrl: computed(() =>
        src.value.indexOf('http') > -1
          ? src.value
          : storeState.videoImgDomain + src.value.replace(/jpeg|jpg|png|gif/, 'html')
      )
    })

    const getImgBase64 = () => {
      if (!storeState.videoImgDomain) return
...
<style scoped lang="scss" src="/@/sass/blocks/imgVideoLoad.scss"></style>
